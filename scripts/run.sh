...
if [ ! -f "${EXE_PATH}" ] && [ "$#" -eq 0 ] && [ -f "${PROJECT_ROOT}/build/release/psx-grid.exe" ]; then
  EXE_PATH="${PROJECT_ROOT}/build/release/psx-grid.exe"
fi
...
EXE_PATH="$(CDPATH= cd -- "$(dirname -- "${EXE_PATH}")" && pwd)/$(basename -- "${EXE_PATH}")"
