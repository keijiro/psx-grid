...
def generate(destination):
    ...
    destination.write_text('// Generated by scripts/generate-assets.py; edit assets/ui/masks.py and jacquard-logo.svg.\n'+
        array('ui_pixels','uint32_t',packed)+array('ui_clut','uint32_t',clut))
    rust_metrics = root / 'rust/src/ui_metrics.rs'
    rust_metrics.write_text('//! Generated atlas glyph advances. Edit assets/ui/masks.py instead.\n\n'+
        '/// Widths of printable ASCII glyphs in atlas order.\n'+
        'pub(crate) const ADVANCE: [u8; 95] = [\n'+
        ''.join('    '+', '.join(str(v) for v in metrics[i:i+25])+',\n' for i in range(0,len(metrics),25))+
        '];\n')
