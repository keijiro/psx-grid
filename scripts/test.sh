...
${CC:-cc} -std=c11 -Wall -Wextra -Werror -g -fsanitize=address,undefined \
  -Iinclude -Isrc -Ibuild/generated tests/audio_test.c src/abi_checks.c src/input/input_queue.c src/audio/sequencer.c "$RUST_LIBRARY" \
  -o build/tests/audio_test
build/tests/audio_test
${CC:-cc} -std=c11 -Wall -Wextra -Werror -g -fsanitize=address,undefined \
  -Iinclude -Isrc -Ibuild/generated tests/synthesis_test.c src/abi_checks.c src/audio/sequencer.c "$RUST_LIBRARY" \
