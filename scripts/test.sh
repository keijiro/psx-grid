...
${CC:-cc} -std=c11 -Wall -Wextra -Werror -g -fsanitize=address,undefined \
  -Isrc tests/storage_editor_test.c src/abi_checks.c src/input/input_queue.c "$RUST_LIBRARY" -o build/tests/storage_editor_test
...
${CC:-cc} -std=c11 -Wall -Wextra -Werror -g -fsanitize=address,undefined \
  -Isrc tests/score_test.c src/abi_checks.c src/input/input_queue.c "$RUST_LIBRARY" -o build/tests/score_test
build/tests/score_test
${CC:-cc} -std=c11 -Wall -Wextra -Werror -g -fsanitize=address,undefined \
  -Itests/gpu_stub -Isrc -Ibuild/generated tests/render_test.c src/ui/render.c src/abi_checks.c "$RUST_LIBRARY" \
  -o build/tests/render_test
build/tests/render_test
${CC:-cc} -std=c11 -Wall -Wextra -Werror -g -fsanitize=address,undefined \
  -Isrc -Ibuild/generated tests/audio_test.c src/abi_checks.c src/input/input_queue.c src/audio/sequencer.c src/audio/audio_abi.c "$RUST_LIBRARY" \
...
${CC:-cc} -std=c11 -Wall -Wextra -Werror -g -fsanitize=address,undefined \
  -Isrc tests/input_test.c src/abi_checks.c src/input/input_queue.c "$RUST_LIBRARY" -o build/tests/input_test
