...
${CC:-cc} -std=c11 -Wall -Wextra -Werror -g -fsanitize=address,undefined \
  -Isrc tests/storage_editor_test.c src/abi_checks.c src/input_queue.c src/ui_format.c "$RUST_LIBRARY" -o build/tests/storage_editor_test
build/tests/storage_editor_test
${CC:-cc} -std=c11 -Wall -Wextra -Werror -g -fsanitize=address,undefined \
  -Isrc tests/replacement_test.c src/abi_checks.c src/sequencer.c "$RUST_LIBRARY" -o build/tests/replacement_test
build/tests/replacement_test
${CC:-cc} -std=c11 -Wall -Wextra -Werror -g -fsanitize=address,undefined \
  -Isrc tests/score_test.c src/abi_checks.c src/input_queue.c src/ui_format.c "$RUST_LIBRARY" -o build/tests/score_test
build/tests/score_test
${CC:-cc} -std=c11 -Wall -Wextra -Werror -g -fsanitize=address,undefined \
  -Itests/gpu_stub -Isrc -Ibuild/generated tests/render_test.c src/render.c src/ui_format.c src/abi_checks.c "$RUST_LIBRARY" \
  -o build/tests/render_test
build/tests/render_test
${CC:-cc} -std=c11 -Wall -Wextra -Werror -g -fsanitize=address,undefined \
  -Isrc -Ibuild/generated tests/audio_test.c src/abi_checks.c src/input_queue.c src/ui_format.c src/sequencer.c src/audio.c "$RUST_LIBRARY" \
  -o build/tests/audio_test
build/tests/audio_test
${CC:-cc} -std=c11 -Wall -Wextra -Werror -g -fsanitize=address,undefined \
  -Isrc -Ibuild/generated tests/synthesis_test.c src/abi_checks.c src/sequencer.c src/audio.c "$RUST_LIBRARY" \
  -lm -o build/tests/synthesis_test
build/tests/synthesis_test
python3 tests/audio_assets_test.py
${CC:-cc} -std=c11 -Wall -Wextra -Werror -g -fsanitize=address,undefined \
  -Isrc tests/input_test.c src/abi_checks.c src/input_queue.c src/ui_format.c "$RUST_LIBRARY" -o build/tests/input_test
