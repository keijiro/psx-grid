...
static void frames(int count) {
    for(int i=0;i<count;i++) {
        ...
        while(pad_read(&sample)) {
            ...
            resumed_cross+=frame.cross; resumed_start+=frame.start;
        }
    }
}
int main(void) {
    editor_init(&editor); input_init(&input); render_init(); audio_platform_init(); pad_init();
    backend=card_platform_backend();
    CardBackend measured=backend; measured.begin=measured_begin; measured.end=measured_end;
    storage_init(&storage,measured);
    score_create(&editor.score,0,0,16);
    for(int i=0;i<SEQUENCER_VOICES;i++) {
        TileValue value=score_default(TILE_NOTE); value.pitch=36+i;
    ...
    editor.mode=EDIT_MAIN; editor.selected=3;
    editor_update(&editor,(InputFrame){.connected=1,.cross=1});
    if(editor.storage_request!=STORAGE_ACTION_SAVE) finish(8);
    storage_fixture_phase=2;
    at=audio_platform_time(); storage_action(1); result=storage.slots[0];
