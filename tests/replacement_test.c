...
typedef struct { EventKind kind; AudioTime at; int pitch; SoundSettings sound; uint32_t token; } Event;
...
static uint32_t note(void *context,AudioTime at,int pitch,SoundSettings sound) {
    (void)context; assert(event_count<(int)(sizeof(events)/sizeof(events[0])));
...
static void reset_trace(void) { event_count=slot=0; }
static void lane(Score *score,int x,int y,int length,int division,int channel,int pitch) {
    ...
    int id=score_at(score,x,y).lane; assert(id>=0);
    assert(!score_set_division(score,id,division));
    assert(!score_set_channel(score,id,channel));
    TileValue value=score_default(TILE_NOTE); value.pitch=pitch;
    assert(!score_place_value(score,x+1,y,value));
}
static int count(EventKind kind,AudioTime at,int pitch) {
    int result=0;
    for(int i=0;i<event_count;i++) if(events[i].kind==kind && events[i].at==at &&
        (kind!=EVENT_ON || events[i].pitch==pitch)) result++;
    return result;
}
...
static void mixed_divisions_and_master(void) {
    Score old,incoming; Sequencer current,prepared;
    score_init(&old);
    ...
    reset_trace(); sequencer_start(&current,&old,trace,0); sequencer_service(&current,0);
...
static void branch_lap_and_gate_transfer(void) {
    Score old,incoming; Sequencer current,prepared;
    score_init(&old); assert(!score_create(&old,0,0,1));
    ...
    TileValue held=score_default(TILE_RELATIVE); held.lock_mask=LOCK_ATTACK; held.attack=100;
    assert(!score_place_value(&old,b->x+1,b->y,held));
    TileValue long_note=score_default(TILE_NOTE); long_note.pitch=60; long_note.length=60;
    assert(!score_place_value(&old,b->x+2,b->y,long_note));
    score_init(&incoming); lane(&incoming,0,0,1,16,0,72);
    reset_trace(); sequencer_start(&current,&old,trace,0); sequencer_service(&current,0);
    sequencer_start(&prepared,&incoming,trace,0); assert(sequencer_replace(&current,&prepared,1));
...
static void conditional_branch_seams(void) {
    ...
    for(int taken=0;taken<=1;taken++) {
        Score old,incoming; Sequencer current,prepared;
        score_init(&old); assert(!score_create(&old,0,0,1));
        TileValue gate=score_default(TILE_CYCLE); gate.period=2; gate.pattern=taken?1:2;
        ...
        assert(!score_resize(&old,branch,2));
        TileValue branch_note=score_default(TILE_NOTE); branch_note.pitch=61;
        assert(!score_place_value(&old,old.lanes[branch].x+2,old.lanes[branch].y,branch_note));

        score_init(&incoming); lane(&incoming,0,0,1,16,0,73);
        ...
        if(taken) {
            ...
            assert(count(EVENT_ON,step,73)==0 && count(EVENT_ON,2*step,73)==0);
        }
...
static void empty_and_stop_adoption(void) {
    Score empty,incoming; Sequencer current,prepared;
    score_init(&empty); score_init(&incoming); lane(&incoming,0,0,1,16,0,65);
    reset_trace(); sequencer_start(&current,&empty,trace,100);
    sequencer_start(&prepared,&incoming,trace,0);
    ...
    Score old; Sequencer stopped,next;
    score_init(&old); lane(&old,0,0,4,16,0,48);
    reset_trace(); sequencer_start(&stopped,&old,trace,0); sequencer_service(&stopped,0);
    sequencer_start(&next,&incoming,trace,0); assert(sequencer_replace(&stopped,&next,1));
    active=sequencer_stop(&stopped,10);
    assert(active==&next && !active->playing && count(EVENT_STOP,10,0)==1);
    sequencer_service(active,10); assert(count(EVENT_ON,10,65)==0);
}

static void split_slice_discovers_seam(void) {
    Score old,incoming; Sequencer current,prepared;
    score_init(&old); assert(!score_create(&old,0,0,1));
    TileValue held=score_default(TILE_RELATIVE); held.lock_mask=LOCK_ATTACK; held.attack=1;
    for(int y=0;y<40;y++) assert(!score_place_value(&old,1,y,held));
    score_init(&incoming); lane(&incoming,0,0,1,16,0,90);
    reset_trace(); sequencer_start(&current,&old,trace,0);
    sequencer_service(&current,0); assert(current.slicing);
    sequencer_start(&prepared,&incoming,trace,0);
    assert(sequencer_replace(&current,&prepared,1));
