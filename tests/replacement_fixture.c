...
static void log_line(const char *line) { *(const char *volatile *)0x1f802084=line; }
...
static void report(const char *name,int a,int b,AudioTime ticks) {
    ...
    snprintf(line,sizeof(line),
        "REPLACEMENT case=%s a=%d b=%d ticks=%u services=%u cost=%u interval=%u dispatch=%u notes=%u playing=%d revision=%u\n",
...
static void frames(int count) {
    for(int i=0;i<count;i++) {
        ...
        InputSample sample; while(pad_read(&sample)) {}
    }
}
static void make_score(Score *score,int bpm,int size,int pitch) {
    score_init(score); score_create(score,0,0,1); score_set_bpm(score,bpm);
    score_set_reverb(score,(ReverbSettings){size,100});
    for(int i=0;i<SEQUENCER_VOICES;i++) {
        if(i) score_create(score,0,i,1);
        TileValue note=score_default(TILE_NOTE); note.pitch=pitch+i; note.length=128;
...
static AudioTime wait_adopt(Score *score) {
    ...
    while(!audio_platform_take_replacement(score)) {
        frames(1);
        if(audio_platform_time()-begin>2*SEQUENCER_HZ) finish(90);
    }
    return audio_platform_time()-begin;
}
static uint32_t reverb_word(void) {
    uint32_t value=0;
    SpuSetTransferStartAddr(REVERB_BASE); SpuRead(&value,sizeof(value));
    SpuIsTransferCompleted(SPU_TRANSFER_WAIT); return value;
}
static void set_reverb_word(uint32_t value) {
    SpuSetTransferStartAddr(REVERB_BASE); SpuWrite(&value,sizeof(value));
...
int main(void) {
    ...
    audio_platform_init(); pad_init(); frames(4);
    log_line("REPLACEMENT checkpoint=init\n");

    make_score(&score,120,0,36);
    log_line("REPLACEMENT checkpoint=score\n");
    audio_platform_update(&score,1,1); frames(4);
    candidate=score; score_set_bpm(&candidate,121);
    ...
    uint16_t mask=IRQ_MASK; IRQ_MASK=mask&~(1u<<IRQ_TIMER0);
    ...
    if(!waiting || score.bpm!=181 || audio_platform_playing()) finish(3);

    audio_platform_update(&score,1,1); frames(3);
    make_score(&candidate,191,1,55);
    if(!audio_platform_replace(&candidate)) finish(4);
    audio_platform_update(&score,0,0);
    waiting=audio_platform_replacing(); elapsed=wait_adopt(&score);
    report("disconnect-adopt",waiting,score.bpm,elapsed);
    if(!waiting || score.bpm!=191 || audio_platform_playing()) finish(5);
    ...
    audio_platform_update(&score,1,1); frames(3);
    uint32_t marker=0x51a7c3e9; set_reverb_word(marker);
    ...
    marker=0x29d46fb1; set_reverb_word(marker);
    ...
    if(audio_note_count<12 || !audio_platform_playing()) finish(10);
    finish(0);
}
