...
static void gates_branches(void) {
    ...
    put(b.x + 1, b.y, v);
    start();
    assert(seq.count == 1 && count == 1 && seq.runners[0].lane == second && first != second);
    sequencer_service(&seq, SEQUENCER_HZ / 8);
    assert(seq.runners[0].lane == child);
    sequencer_service(&seq, SEQUENCER_HZ / 4);
    assert(events[1].pitch == 72 && seq.runners[0].lane == 0 && seq.runners[0].lap == 1);
    sequencer_service(&seq, 3 * SEQUENCER_HZ / 8);
...
static void voices(void) {
    ...
    base(2);
    score.sounds[0] = (SoundSettings){0, 500, WAVE_SINE, WAVE_SINE, 0, 0, 0, 200, 0};
    put(1, 0, score_default(TILE_NOTE));
...
static void model_editor(void) {
    ...
    snapshot = score;
    v.attack = 16001;
    assert(score_edit(&score, id, v) == SCORE_INVALID && !memcmp(&score, &snapshot, sizeof(score)));
    v = relative(1, 2);
    v.lock_mask = 0;
