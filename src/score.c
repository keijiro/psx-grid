...
ScoreResult score_validate_import(const Score *s) {
    ...
    for (int i = 0; i < SCORE_LANES; i++)
        if (s->lanes[i].active) {
            ...
            for (int d = 0; d < SCORE_DIVISIONS; d++)
                if (l->division == score_divisions[d])
                    division = 1;
            if (!division)
                return SCORE_INVALID;
            for (int j = 0; j < SCORE_STEPS; j++) {
                if (j >= l->length && l->tiles[j])
                    return SCORE_INVALID;
                int depth = 0;
                for (TileId t = l->tiles[j]; t;) {
                    ...
                    if (v->value.kind == TILE_JUMP &&
                        (!valid(s, v->branch) || s->lanes[v->branch].source != t))
                        return SCORE_INVALID;
                    t = v->next;
                }
            }
    ...
    for (int i = 0; i < SCORE_LANES; i++)
        if (s->lanes[i].active && s->lanes[i].source) {
            ...
            if (!seen[t] || s->tiles[t].value.kind != TILE_JUMP || s->tiles[t].branch != i)
                return SCORE_INVALID;
        }
    for (int t = 1; t <= SCORE_TILE_CAPACITY; t++)
        if (!!s->tiles[t].value.kind != !!seen[t])
            return SCORE_INVALID;
    return validate(s);
}
...
static ScoreResult insert(Score *s, Cell c, TileValue v) {
    ...
    TileId id = 1;
    while (id <= SCORE_TILE_CAPACITY && s->tiles[id].value.kind)
        id++;
...
static ScoreResult move(const Score *s, int sx, int sy, int x, int y) {
    ...
    if (same) {
        ...
        *from = 0;
        while (scratch.tiles[tail].next)
            tail = scratch.tiles[tail].next;
    }
    if (b.kind == CELL_END) {
        if (scratch.lanes[b.lane].length == SCORE_STEPS)
            return SCORE_BOUNDS;
...
ScoreResult score_set_sound(Score *s, int channel, SoundSettings sound) {
    ...
    if (sound.wave_a < 0 || sound.wave_a >= WAVE_COUNT || sound.wave_b < 0 ||
        sound.wave_b >= WAVE_COUNT || sound.mix_attack < 0 || sound.mix_attack > SOUND_MAX_MIX_MS ||
        sound.mix_release < 0 || sound.mix_release > SOUND_MAX_MIX_MS ||
        sound.sweep < -SOUND_MAX_SWEEP || sound.sweep > SOUND_MAX_SWEEP || sound.decay < 0 ||
        sound.decay > SOUND_MAX_DECAY_MS)
        return SCORE_INVALID;
