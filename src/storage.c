...
static int identity(const CardFile* file, int* slot, uint32_t* generation)
{
    ...
    if (memcmp(name, "BIJACQUARD", 10) || name[20] || name[10] < '0' ||
        name[10] > '1' || name[11] < '0' || name[11] > '9')
    {
        return 0;
    }
...
static CardResult read_file(Storage* s, const char* name)
{
    ...
    if (r) return r;
    CardResult transfer =
        s->card.read(s->card.context, s->readback, SCORE_FILE_BYTES);
...
static CardResult write_file(Storage* s, const char* name)
{
    ...
    if (r) return r;
    CardResult transfer =
        s->card.write(s->card.context, s->save, SCORE_FILE_BYTES);
    CardResult closed = s->card.close(s->card.context);
    if (transfer) return transfer;
    if (closed) return closed;
    return s->card.check(s->card.context);
...
static Discovery discover(Storage* s, int slot)
{
    ...
    for (int i = 0; i < s->file_count; i++)
    {
        ...
        if (r)
        {
            found.result = card_result(r);
            return found;
        }
        int payload_slot;
        uint32_t payload_generation;
        FormatResult format =
            score_format_decode(s->readback, SCORE_FILE_BYTES, &s->incoming,
                                &payload_slot, &payload_generation);
        if (format == FORMAT_NEWER)
        {
            if (generation > newer) newer = generation;
            continue;
        }
        if (format != FORMAT_OK || payload_slot != slot ||
            payload_generation != generation)
        {
            if (found.result == STORAGE_EMPTY) found.result = STORAGE_CORRUPT;
            continue;
        }
        // Enumeration order is the BIOS directory order. The first valid entry
        // wins ties, as the former lower-directory-index rule did.
...
StorageResult storage_load(Storage* s, int slot)
{
    ...
    FormatResult format = score_format_decode(
        s->readback, SCORE_FILE_BYTES, &s->incoming, &number, &generation);
    if (format != FORMAT_OK || number != slot || generation != d.generation)
    {
        return finish(s, slot,
                      format == FORMAT_NEWER ? STORAGE_NEWER : STORAGE_CORRUPT);
    }
...
static CardResult retire(Storage* s, int slot, int keep)
{
    for (int i = 0; i < s->file_count; i++)
    {
        if (i != keep)
        {
            int number;
            uint32_t generation;
            if (!identity(&s->files[i], &number, &generation) ||
                number != slot || s->files[i].size != SCORE_FILE_BYTES)
            {
                continue;
            }
            CardResult r = s->card.check(s->card.context);
...
StorageResult storage_save(Storage* s, int slot, const Score* score)
{
    ...
    if (r != STORAGE_SAVED) return s->slots[slot - 1] = r;
    Discovery d = discover(s, slot);
    if (d.result != STORAGE_EMPTY && d.result != STORAGE_SAVED &&
        d.result != STORAGE_CORRUPT)
    {
        return finish(s, slot, d.result);
    }
    if (d.maximum == UINT32_MAX)
    {
        return finish(s, slot, STORAGE_GENERATION_FULL);
    }
    ...
    char name[21];
    snprintf(name, sizeof(name), "BIJACQUARD%02d%08lX", slot,
    ...
    FormatResult format = score_format_decode(
        s->readback, SCORE_FILE_BYTES, &s->incoming, &number, &read_generation);
    ...
    io = retire(s, slot, -1);
    if (io == CARD_CHANGED || io == CARD_MISSING)
    {
        return finish(s, slot, card_result(io));
    }
