...
static Discovery discover(Storage* s, int slot)
{
    ...
    for (int i = 0; i < s->file_count; i++) {
        ...
        if (!identity(&s->files[i], &number, &generation) || number != slot)
            continue;
        if (generation > found.maximum)
            found.maximum = generation;
    ...
    if (newer >= found.generation && newer)
...
StorageResult storage_load(Storage* s, int slot)
{
    ...
    uint32_t generation;
    FormatResult format =
        score_format_decode(s->readback, SCORE_FILE_BYTES, &s->incoming, &number, &generation);
...
static CardResult retire(Storage* s, int slot, int keep)
{
    for (int i = 0; i < s->file_count; i++)
        if (i != keep) {
            ...
            uint32_t generation;
            if (!identity(&s->files[i], &number, &generation) || number != slot ||
                s->files[i].size != SCORE_FILE_BYTES)
                continue;
            CardResult r = s->card.check(s->card.context);
            if (r)
                return r;
...
StorageResult storage_save(Storage* s, int slot, const Score* score)
{
    ...
    if (!s->free_blocks)
        return finish(s, slot, STORAGE_NO_SPACE);
    ...
    FormatResult format =
        score_format_decode(s->readback, SCORE_FILE_BYTES, &s->incoming, &number, &read_generation);
