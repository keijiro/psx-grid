...
static int identity(const CardFile* file, int* slot, uint32_t* generation)
{
    ...
    if (memcmp(name, "BIJACQUARD", 10) || name[20] || name[10] < '0' || name[10] > '1' || name[11] < '0' ||
        name[11] > '9')
        return 0;
...
static Discovery discover(Storage* s, int slot)
{
    ...
    for (int i = 0; i < s->file_count; i++)
    {
        ...
        FormatResult format =
            score_format_decode(s->readback, SCORE_FILE_BYTES, &s->incoming, &payload_slot, &payload_generation);
...
static StorageResult begin(Storage* s)
{
    ...
    for (int i = 0; i <= 15; i++)
    {
        ...
        if (i == 15 || file.size <= 0 || file.size % SCORE_FILE_BYTES || used + file.size / SCORE_FILE_BYTES > 15)
        {
            s->card.end(s->card.context);
            return STORAGE_CARD_DAMAGED;
        }
...
StorageResult storage_load(Storage* s, int slot)
{
    ...
    int number;
    uint32_t generation;
    FormatResult format = score_format_decode(s->readback, SCORE_FILE_BYTES, &s->incoming, &number, &generation);
...
static CardResult retire(Storage* s, int slot, int keep)
{
    for (int i = 0; i < s->file_count; i++)
    {
        if (i != keep)
        {
            int number;
            uint32_t generation;
            if (!identity(&s->files[i], &number, &generation) || number != slot || s->files[i].size != SCORE_FILE_BYTES)
            {
                continue;
            }
            CardResult r = s->card.check(s->card.context);
...
StorageResult storage_save(Storage* s, int slot, const Score* score)
{
    ...
    FormatResult format = score_format_decode(s->readback, SCORE_FILE_BYTES, &s->incoming, &number, &read_generation);
    if (memcmp(s->save, s->readback, SCORE_FILE_BYTES) || format != FORMAT_OK || number != slot ||
...
const char* storage_message(StorageResult r)
{
    static const char* messages[] = {
        "CHECK",          "EMPTY",        "SAVED",         "BUSY",         "CORRUPT",
        "NEWER VERSION",  "NO CARD",      "CARD TIMEOUT",  "CARD CHANGED", "CARD I/O ERROR",
        "UNFORMATTED",    "CARD DAMAGED", "NO FREE BLOCK", "SCORE FULL",   "SAVED / CLEANUP PENDING",
        "GENERATION FULL"};
    return messages[r];
}
