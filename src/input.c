...
void input_reset_value_repeat(Input *i) { i->value_direction=0; i->value_held=0; i->value_countdown=0; }
void input_reset_repeat(Input *i) { i->countdown = INPUT_DELAY; i->row_direction=0; i->row_countdown=0; input_reset_value_repeat(i); }
InputFrame input_update(Input *i, int connected, uint16_t held) {
    ...
    i->previous = held;
    if(!row_dy) { i->row_direction=0; i->row_countdown=0; }
    else if(row_dy!=i->row_direction) {
        i->row_direction=row_dy; i->row_countdown=INPUT_DELAY; f.row_dy=row_dy;
    } else if(--i->row_countdown<=0) {
        i->row_countdown=INPUT_INTERVAL; f.row_dy=row_dy;
    }
    ...
    if(!value) { i->value_direction=0; i->value_held=0; i->value_countdown=0; }
    else if(value!=i->value_direction || !!coarse!=i->value_coarse) {
        i->value_direction=value; i->value_coarse=!!coarse;
        i->value_held=0; i->value_countdown=INPUT_VALUE_DELAY;
        f.value_dir=value; f.value_coarse=!!coarse;
    } else {
        i->value_held++;
