...
typedef enum { TILE_NONE, TILE_NOTE, TILE_CYCLE, TILE_PROBABILITY, TILE_JUMP, TILE_KIND_COUNT } TileKind;
...
typedef struct { TileKind kind; int pitch, length, period, chance; uint32_t pattern; } TileValue;
...
typedef struct { Lane lanes[SCORE_LANES]; Tile tiles[SCORE_TILE_CAPACITY + 1]; } Score;
