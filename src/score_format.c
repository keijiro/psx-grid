...
FormatResult score_format_encode(const Score *s,
    ...
    if (used > SCORE_FILE_BYTES)
        return FORMAT_FULL;
    if (slot < 1 || slot > 15 || !generation)
        return FORMAT_CORRUPT;
...
score_format_decode(const uint8_t *block, size_t size, Score *s, int *slot, uint32_t *generation) {
    ...
    if (header < HEADER || header > size - SCORE_FILE_WRAPPER ||
        ...
        return FORMAT_CORRUPT;
    if (get(h + 20, 4) != checksum(h, header + bytes))
        return FORMAT_CORRUPT;
    if (get(h + 4, 2) != 1 || get(h + 6, 2) > 1 || get(h + 16, 4) || header != HEADER)
        return FORMAT_NEWER;
    ...
    for (int i = 0; i < SCORE_CHANNELS; i++) {
        ...
        if (wave_value(p[4]) < 0 || wave_value(p[5]) < 0)
            return FORMAT_NEWER;
        SoundSettings v = {(int)get(p, 2),
                           (int)get(p + 2, 2),
                           wave_value(p[4]),
                           wave_value(p[5]),
                           (int)get(p + 6, 2),
                           ...
                           (int)get(p + 11, 2),
                           p[13]};
        if (!zero(p + 14, 2) || score_set_sound(s, i, v))
            return FORMAT_CORRUPT;
    }
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (!zero(p + 6, 6) || p[5] > 1)
            return FORMAT_CORRUPT;
        if ((p[5] && (p[3] || p[4] != 255)) || (!p[5] && p[4] >= SCORE_CHANNELS))
            return FORMAT_CORRUPT;
        ...
        l->channel = p[5] ? -1 : p[4];
        roles[i] = p[5];
        if (!p[5] && score_set_division(s, i, p[3]))
            return FORMAT_CORRUPT;
    }
    p = chunks[STEPS];
            ...
            for (int k = 0; k < n; k++) {
                ...
                *link = id;
                ...
                case TILE_PROBABILITY:
                    ...
                    break;
                case TILE_JUMP:
                    if (p[0] >= count || !roles[p[0]] || s->lanes[p[0]].source)
                        return FORMAT_CORRUPT;
                    t->branch = p[0];
    ...
    if (p != end)
        return FORMAT_CORRUPT;
    for (int i = 0; i < count; i++)
        if (roles[i] != !!s->lanes[i].source)
            return FORMAT_CORRUPT;
    if (score_validate_import(s) != SCORE_OK || score_format_measure(s) > SCORE_FILE_BYTES)
        return FORMAT_CORRUPT;
    s->generation = 0;
    for (int i = 0; i < count; i++)
        s->lane_generation[i] = ++s->generation;
    for (TileId t = 1; t < next; t++)
        s->tile_generation[t] = ++s->generation;
    s->revision = 1;
    if (slot)
        *slot = h[28];
    if (generation)
        *generation = get(h + 24, 4);
    return FORMAT_OK;
}
